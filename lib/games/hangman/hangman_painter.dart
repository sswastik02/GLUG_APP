...
import 'package:glug_app/games/hangman/enum_collection.dart';
import 'package:glug_app/games/hangman/gamebloc.dart';
...
class VertHangManPainter extends CustomPainter {
  ...
  _drawHead(Canvas canvas, Size size, Paint paint) {
    ...
    var neckEnd =
        Offset(size.width / 2, size.height / 5 + _headHeight + _headHeight / 3);
  ...
  _drawBody(Canvas canvas, Size size, Paint paint) {
    ...
    canvas.drawRect(
        Rect.fromCenter(
            center: Offset(size.width / 2, (neckEnd.dy + bodyEnd.dy) / 2),
